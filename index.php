...
$tasks = array(
    ...
    29  => array(
        ...
        "answers"  => array(
            ...
            4 => "An order doesn't have a status, only a statStatus is a property of an invoice, shipment, and credit memo."
        )
    ),
    30  => array(
    ...
    35  => array(
        "question" => "The checkbox 'Use default value' is checked and the corresponding input field is disabled, as shown in the graphic abovHow does Magento represent this state in the corresponding EAV value tables?",
        "answers"  => array(
    ...
    36  => array(
        "question" => "You want to define a list of quote object attributes that are copied to the order object when an order is placeWhich one of the following statements is true?",
        "answers"  => array(
    ...
    59  => array(
        ...
        "answers" => array(
            1 => 'Declare your module`s block class prefix in the config.xml fil',
    ...
    69  => array(
        "question" => 'Assume that PayPal has presented a new API for online purchasing that you are going to usWhich class should you extend for doing that?',
        "answers" => array(
    ...
    77  => array(
        "question" => 'You have used the "refresh" action on the cache management page for the block html cache typWhich of the following types of cache record would be cleared?',
        "answers" => array(
    ...
    79  => array(
        "question" => 'Assume you have product with qty in stock=1, and two people are trying to buy it at the same timWhich of the following statements is true?',
        "answers" => array(
    ...
    82  => array(
        "question" => 'Two modules, Foo and Moo, both rewrite the catalog/product model,but only the Moo module`s rewrite is being useBoth modules Foo and Moo reside in the community code pool. Which two approaches allow the functionality from both classes to be used? (Choose two)',
        "answers" => array(
    ...
    97  => array(
        "question" => 'A customer bought two items but wants to cancel onWhich option enables the administrator of the site to edit the quantity while the invoice is being captured in the admin area?',
        "answers" => array(
